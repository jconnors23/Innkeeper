...
client.on('messageCreate', async (message) => {
    try {
        ...
        let response;
        try {
            response = await axios.get('https://api.scryfall.com/cards/search?q=' + "!" + "\"" + query + "\"")
        } catch (error: any) {
        ...
        if (card.image_uris) {
            ...
            cardimages.push(card.card_faces![0].image_uris!.small, card.card_faces![1].image_uris!.small);
        }
